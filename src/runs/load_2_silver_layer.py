# Auto-generated by load_2_silver_layer_builder_agent.py
# run_id: 20260124_231016_#6470e523
# model: gpt-4.1
# template_sha256: bc38173e4f302947177953166520a0b2b7e4d1d14c53010170a2444c3f46c581
# agent_context_sha256: 21117ff1fefb7c2ba2ca20da6ee373ad77a95eb96c2ebf89c5913409a16cffc3
# human_report_sha256: 43ac5769b4dc990ef56584a1dced210f1ff49096841e919e156f7b4f0f0ff00d
...
def transform_customer_info(df: pd.DataFrame) -> pd.DataFrame:
    """
    Silver logic for customer_info.csv:
    - Apply base cleaning
    - Normalize customer_id to Int64
    - Parse date_of_birth as date
    - Standardize gender codes (M/F/NA)
    - Trim whitespace in firstname, lastname
    """
    df = base_silver_cleaning(df)
    if "customer_id" in df.columns:
        df = normalize_integer_id(df, "customer_id")
    if "date_of_birth" in df.columns:
        df = normalize_date_column(df, "date_of_birth")
    if "gender" in df.columns:
        tmp = df["gender"].astype("string").str.strip().str.upper()
        tmp = tmp.replace({
            "MALE": "M",
            "FEMALE": "F",
            "F": "F",
...
def transform_product_info(df: pd.DataFrame) -> pd.DataFrame:
    """
    Silver logic for product_info.csv:
    - Apply base cleaning
    - Normalize product_id to Int64
    - Normalize price to float
    - Trim whitespace in product_name, category
    """
    df = base_silver_cleaning(df)
    if "product_id" in df.columns:
        df = normalize_integer_id(df, "product_id")
    if "price" in df.columns:
        df = normalize_numeric_column(df, "price")
...
def transform_sales_transactions(df: pd.DataFrame) -> pd.DataFrame:
    """
    Silver logic for sales_transactions.csv:
    - Apply base cleaning
    - Normalize transaction_id, customer_id, product_id, quantity to Int64
    - Normalize unit_price to float
    - Parse transaction_date as date
    """
    df = base_silver_cleaning(df)
    for col in ["transaction_id", "customer_id", "product_id", "quantity"]:
        if col in df.columns:
            df = normalize_integer_id(df, col)
    if "unit_price" in df.columns:
        df = normalize_numeric_column(df, "unit_price")
    if "transaction_date" in df.columns:
        df = normalize_date_column(df, "transaction_date")
    # Standardize missing values in all columns
    for col in df.columns:
        df[col] = df[col].replace({"": pd.NA, "nan": pd.NA, "NaN": pd.NA, "null": pd.NA, "NULL": pd.NA})
    return df
...
def transform_for_silver(filename: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if name == "customer_info.csv":
        return transform_customer_info(df)
    if name == "product_info.csv":
        return transform_product_info(df)
    if name == "sales_transactions.csv":
        return transform_sales_transactions(df)
