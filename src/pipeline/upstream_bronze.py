"""
File: src/pipeline/upstream_bronze.py
Purpose:
  This module contains the original implementation of the Bronze loader from
  the upstream repository.  It is imported by `bronze_layer.py` to perform
  the actual ingestion of raw CRM and ERP CSV files into a run‑scoped
  directory.  The code here should not be modified unless upstream logic
  changes.  See `bronze_layer.py` for the wrapper that integrates this
  module into the unified run pipeline.

Inputs:
  - raw/source_crm/*.csv
  - raw/source_erp/*.csv
Outputs:
  - artifacts/<bronze_root>/<run_id>/... (see upstream code)

Step:
  Bronze (ELT ingest)
"""

# NOTE: The following code is a verbatim copy of the upstream
# NOTE: load_1_bronze_layer.py.  It implements the Bronze layer logic.
